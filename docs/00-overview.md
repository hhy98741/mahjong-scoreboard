...
| D8b | **The points table and the selectable band are separate settings, and live in different places.** `table_max_faan` is a **ruleset** field setting how far the price list goes (0–13). `min_faan`/`max_faan` are **game** fields set on the New Game screen, defaulting to **2 and 8**. Narrowing the band never deletes points rows. | The price list rarely changes; the band varies night to night, so it belongs with the other per-game choices. |
...
| D25 | **Player count is a reporting filter, defaulting to 4.** Rate statistics are not comparable across player counts. | A 2-player win rate is structurally ~1/2; a 4-player one ~1/4. |
